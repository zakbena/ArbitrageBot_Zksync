...
async function main() {
  ...
  if (chainId == 31337) {
    ...
    flashswapContract = await ethers.getContract("UniswapV2FlashSwap", wallet);
    deployer = accounts[0];
  } else {
    ...
    tokenContract = new Contract(tokenBorrowedAddress, usualABI, wallet);
  }
  ...
  let flashSwapDone = await flashswapContract.flashSwap(
